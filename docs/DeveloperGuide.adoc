...
.. _System Tests_ that test the entire App by simulating user actions on the GUI. These are in the `guisystemtests` package.
...
*** The system tests link:{repoURL}/src/test/java/guisystemtests/AddressBookSystemTest.java[`AddressBookSystemTest#assertCommandBoxShowsDefaultStyle() and AddressBookSystemTest#assertCommandBoxShowsErrorStyle()`] to include verification for `ResultDisplay` as well.
...
** For system tests, modify link:{repoURL}/src/test/java/guisystemtests/AddressBookSystemTest.java[`AddressBookSystemTest`] to also verify the new total number of employees status bar.
