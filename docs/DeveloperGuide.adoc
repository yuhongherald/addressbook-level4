...
.Structure of Logic with SetCommand implemented
image::SetCommandLogicDiagram.png[width="800"]

.Structure of Model with SetCommand implemented
...
. Add a shorthand equivalent <<alias, alias>> for each of the individual commands. For example, besides typing `clear`, the user can also type `c` to remove all employees in the list.
