...
As you can see from figure 11, AddressBookParser makes a reference to CommandWords. However, `CommandWords` resides in `UserPrefs` in the `Model` component. This is to facilitate the saving of data. CommandWords also has a method checkIntegrity, which ensures that object command words are 1 to 1 with class command words. This allows forward compatibility, as well as protects against data corruption to some extent.
...
// tag::importexport[]
=== Import/Save feature #Coming in v2.0#
==== Current Implementation
...
* Figure 12 below shows the process of initializing ImportSession. On the `import` command,
...
write comments.
2. `JobEntry`: Augmented `Job` that implements `ExcelRowReference` to retrieve the sheet number
and row number for writing into the excel file. `JobEntry` also contains the `Job` data which
will be imported upon approval.
+
.Sequence Diagram of `ImportSession` initialization
image::SessionInitializationSequenceDiagram.png[width="800"]

* Figure 13 below shows the process of reviewing job entries in an import session. On the `review`
command, `sessionData` updates the relevant `jobEntry` and retrieves the sheet and row number,
which is used to update `sheetWithHeaderFields`.
+
.Sequence Digram of `ImportSession` during the reviewing of a job
image::SessionReviewSequenceDiagram.png[width="800"]
+
* Figure 14 shows the process of saving a session. On the `save` command, `importSession` writes the
...
ImportSession alone or Import+Session
addJob/addEmployees in model or storage

==== Design Considerations
Supporting UndoableCommand

* **Current choice:** Add the previous instance of `sessionData` when setting data for `Command`, with an
...
* _Alternative:_ Make `ImportSession` not a `Singleton` class and store it directly.
