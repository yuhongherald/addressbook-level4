...
Accepts a job with index NUMBER in imported list.
...
** Example 1: +
. `accept 1` +
*** Accepts the job entry with index 1 and removes it from imported list.
...
Rejects a job with index NUMBER in imported list.
...
** Example 1: +
. `reject 1` +
*** Rejects job entry with index 1 and removes it from imported list. It is not added to CarviciM.
