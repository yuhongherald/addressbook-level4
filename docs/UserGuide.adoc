...
You will see the sorted list of employee after the execution of `sort` command (shown in the image below).
...
You will need to login to Gmail before you can use the `email` feature to send out emails to your employees (see Section 5.19 below). To login to Gmail, use the `login` command as follows: +
Format: `login`
...
[NOTE]
====
When you are redirected to the Gmail login page, CarviciM may sometimes appear as if it is not responding. Don't panic! Once you have successfully logged in to Gmail, CarviciM will be up and running again, from exactly where you left off.
====
...
To discretely import job entries, you must first import a file, then <<accept-reject, `accept` and `reject`>>
job entries.
...
You don't have to worry if you forgot your custom words! They will displayed in the result display
