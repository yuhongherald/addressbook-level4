...
* **`adde`**`n/John Doe p/98765432 e/johnd@example.com` : adds an employee named `John Doe` to the Car Servicing Manager.
...
Adds a job entry to the car servicing manager +
...
Closes an ongoing job entry in the car servicing manager based on the specified job number +
...
Adds an employee to the car servicing manager +
...
Shows a list of all employees in the car servicing manager . +
Format: `liste`

=== Editing a employee : `edite`

Edits an existing employee in the car servicing manager . +
...
Deletes the specified employee from the car servicing manager. +
...
* `liste` +
`deletee 2` +
Deletes the 2nd employee in the car servicing manager.
* `finde Betsy` +
`deletee 1` +
...
e.g. `deletee 3`
* *Edit* : `edite INDEX [p/PHONE_NUMBER] [e/EMAIL] [t/TAG]...` +
e.g. `edite 2 e/jameslee@example.com`
* *Find Employee* : `finde KEYWORD [MORE_KEYWORDS]` +
